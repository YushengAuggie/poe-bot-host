...
class MockQueryWithAttachment(QueryRequest):
    ...
    def create_with_attachment(cls, message: str, image_path: str) -> "MockQueryWithAttachment":
        ...
        attachment = Attachment(url=f"file://{filename}", name=filename, content_type=content_type)
        ...
        protocol_message = ProtocolMessage(role="user", content=message, attachments=[attachment])
    ...
    if hasattr(query.query[-1], "attachments"):
        ...
        for i, attachment in enumerate(attachments):
            ...
            if "content" in attachment.__dict__:
                logger.info(
                    f"Attachment {i+1} content size: {len(attachment.__dict__['content'])} bytes"
                )
    ...
    def patched_extract_attachments(self, query):
        ...
        for attachment in attachments:
            ...
            if patched:
                ...
                logger.info(
                    f"Attachment now has content attribute: {hasattr(attachment, 'content')}"
                )
...
async def test_with_patch(image_path: str, message: str):
    ...
    logger.info("Created mock query with attachment")
    ...
    logger.info(
        f"First unpatched response: {unpatched_responses[0] if unpatched_responses else 'None'}"
    )
    logger.info(f"Patched responses: {len(patched_responses)}")
    logger.info(
        f"First patched response: {patched_responses[0][:100] if patched_responses else 'None'}..."
    )
