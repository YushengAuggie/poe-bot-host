...
def test_bot_image_handling(image_path: str):
    ...
    query = QueryRequest(
        ...
        type="query",
        query=[
            ProtocolMessage(role="user", content="What is in this image?", attachments=[attachment])
        ],
        user_id="test_user",
    ...
    print(
        "To actually get results from the API, you need to set the GOOGLE_API_KEY environment variable."
    )
