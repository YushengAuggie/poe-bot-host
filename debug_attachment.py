...
def create_mock_attachment(image_path: str) -> Attachment:
    ...
    mock_attachment = Attachment(url=f"file://{filename}", name=filename, content_type=content_type)
...
async def test_bot_with_attachment(image_path: str, message: str):
    ...
    if attachments:
        ...
        if media_data:
            logger.info(
                f"Processed media data: {media_data['mime_type']}, {len(media_data['data'])} bytes"
            )
