...
from fastapi_poe.types import PartialResponse, QueryRequest, Attachment
...
class FileAnalyzerBot(BaseBot):
    ...
    bot_description = "A bot that analyzes uploaded files. Upload a file and I'll provide information about it."
    version = "1.0.0"
    
    # Supported file types
    TEXT_EXTENSIONS = [".txt", ".md", ".csv", ".json", ".yaml", ".yml"]
    CODE_EXTENSIONS = [".py", ".js", ".html", ".css", ".java", ".c", ".cpp", ".go", ".rs", ".php", ".rb"]
    ...
    def _extract_file_content(self, attachment: Attachment) -> str:
        ...
        try:
            ...
            if ext not in supported_extensions:
                raise BotErrorNoRetry(f"Unsupported file type: {ext}. Supported types: {', '.join(supported_extensions)}")
            ...
            raise BotErrorNoRetry("Unable to decode file. Make sure it's a text file with UTF-8 encoding.")
    ...
    def _analyze_csv(self, content: str) -> Dict[str, Any]:
        ...
        return {
            "columns": columns,
            "rows": len(non_empty_lines) - 1  # Excluding header
        }
    ...
    def _format_analysis(self, analysis: Dict[str, Any]) -> str:
        ...
        if analysis["has_content"]:
            ...
            lines = '\n'.join(content.split('\n')[:10]) if 'content' in locals() else "Content preview not available"
            response += f"{lines[:500]}"
            response += "...\n```\n"
    ...
    async def _process_message(self, message: str, query: QueryRequest) -> AsyncGenerator[PartialResponse, None]:
        ...
        attachments = query.query[-1].get("attachments", []) if isinstance(query.query, list) and query.query else []
...
""")
        ...
        if not attachments:
            yield PartialResponse(text="Please upload a file for me to analyze. Type 'help' for instructions.")
            return
