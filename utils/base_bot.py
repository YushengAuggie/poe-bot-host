...
class BaseBot(PoeBot):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # If no access key was provided, try to get it from environment variables
        ...
        # Make sure we use class attribute as fallback for bot_name
        if not hasattr(self, "bot_name") or self.bot_name is None:
            if hasattr(self.__class__, "bot_name"):
                self.bot_name = self.__class__.bot_name
            else:
                self.bot_name = self.__class__.__name__
